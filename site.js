(function(){
  const pad=n=>String(n).padStart(2,'0'); document.querySelectorAll('[data-demo-date]').forEach(el=>{const d=new Date();d.setDate(d.getDate()+Number(el.dataset.demoDate));el.textContent=`${d.getMonth()+1}월 ${d.getDate()}일`}); document.querySelectorAll('[data-demo-days]').forEach(el=>el.textContent=`${el.dataset.demoDays}일 남음`);
  ...
  const brands=[['ChatGPT','#10A37F'],['Claude','#D97757','anthropic'],['Cursor','#111114'],['Midjourney','#5B5BD6'],['Google AI Pro','#4285F4','googlegemini'],['Perplexity','#1F6FEB','perplexity'],['GitHub Copilot','#24292F','githubcopilot'],['Notion','#111114','notion'],['Canva','#00C4CC'],['Figma','#A259FF','figma'],['ElevenLabs','#111114','elevenlabs'],['Replit','#F26207','replit'],['Adobe','#FA0F00'],['Runway','#111114'],['Framer','#0055FF','framer'],['Webflow','#146EF5','webflow'],['Zapier','#FF4F00','zapier'],['Linear','#5E6AD2','linear']];
  const brandRow=document.getElementById('brandRow');
  if(brandRow) brands.forEach(([name,color,icon])=>{const b=document.createElement('span');b.className='b';b.innerHTML=`<i class="avatar" style="background:${color}">${icon?`<b class="logo" style="-webkit-mask-image:url(./logos/${icon}.svg);mask-image:url(./logos/${icon}.svg);width:14px;height:14px"></b>`:name.slice(0,1)}</i>${name}`;brandRow.append(b)});
  ...
  if(serviceSelect){
    ...
    const guideLabel=document.querySelector('.service-links span'); if(guideLabel) guideLabel.textContent='서비스별 안내';
  }
