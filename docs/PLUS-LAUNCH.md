> [2026-09-16 갱신] Donna Plus는 3개월마다 6,000원이 자동 결제되는 빌링 상품이다.
> 결제 7일 전과 3일 전에 이메일로 안내하며 관리 링크에서 언제든 다음 결제를 해지할 수 있다.
> 사이트·약관·확장·서버의 표시와 동작은 이 정책으로 통일한다. 통신판매업신고는 제2026-경기양평-9103호다.

# Donna Plus 도입 순서 — 토스페이먼츠 + Railway

코드는 다 있다(`server/`, `plus.html`, `plus-manage.html`, 확장 1.7.0). 이 문서는 **사람이 손으로 해야 하는 일**을 순서대로 적은 것이다. 위에서부터 하면 된다. 예상 소요: 계약 심사를 빼면 반나절.

## 0. 준비물

- 토스페이먼츠 가입에 필요한 것: 사업자등록증(659-13-02509), 통신판매업신고증(2026-경기양평-9103), 대표자 신분증, 정산 계좌(사업자 명의), 서비스 URL(https://donna.co.kr), 이용약관·개인정보처리방침 URL(이미 있음).
...
1. Chrome에 확장 1.7.0을 압축해제 로드. 구독 3개 등록 → 4번째에서 Plus 시트 → **Plus 시작하기** → `donna.co.kr/plus.html?install=…` 열림.
...
- 확장은 패널을 열 때와 12시간마다 `/v1/license`를 부른다. 결제 직후 Plus 화면에서는 3초마다 최대 2분간 확인한다. 서버가 죽어도 7일은 마지막 상태를 유지하므로 배포 몇 분 중단은 사용자에게 보이지 않는다.
