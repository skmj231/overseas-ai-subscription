/* Donna · presets.js — 자주 쓰는 해외 AI·SaaS 서비스의 기본값
 *
...
(function (root) {
  ...
  const P = [
    ...
    { id: "claude", name: "Claude Pro", aliases: ["claude", "anthropic", "클로드"],
      color: "#D97757", logo: "anthropic.svg",
    ...
    { id: "gemini", name: "Google AI Pro (Gemini)", short: "Google AI Pro", aliases: ["gemini", "google ai pro", "제미나이", "google one"],
      color: "#4285F4", logo: "googlegemini.svg",
    ...
    { id: "perplexity", name: "Perplexity Pro", short: "Perplexity", aliases: ["perplexity", "퍼플렉시티"],
      color: "#1F6FEB", logo: "perplexity.svg",
      amount: 20, currency: "USD", interval: "month", yearly: 200,
    ...
    { id: "copilot", name: "GitHub Copilot Pro", short: "Copilot Pro", aliases: ["github copilot", "copilot pro", "코파일럿"],
      color: "#24292F", logo: "githubcopilot.svg",
    ...
    { id: "m365", name: "Microsoft 365 Premium (Copilot)", short: "Microsoft 365", aliases: ["microsoft 365", "copilot pro", "m365", "오피스"],
      color: "#0F6CBD", logo: null,
    ...
    { id: "figma", name: "Figma Professional", short: "Figma Pro", aliases: ["figma", "피그마"],
      color: "#A259FF", logo: "figma.svg",
    ...
    { id: "elevenlabs", name: "ElevenLabs", aliases: ["elevenlabs", "eleven labs", "일레븐랩스"],
      color: "#111114", logo: "elevenlabs.svg",
    ...
    { id: "zapier", name: "Zapier Professional", short: "Zapier Pro", aliases: ["zapier", "재피어"],
      color: "#FF4F00", logo: "zapier.svg",
  ...
  /* 로고: logos/ 폴더의 단색 SVG(SimpleIcons 규격)를 CSS mask로 흰색으로 칠해 브랜드색 원 위에 얹는다.
     파일이 없는 서비스는 이름 첫 글자로 대신한다. 새 로고는 logos/에 SVG를 넣고 logo 값만 채우면 된다. */
  function avatarHtml(p, size) {
    const sz = size || 44;
    const bg = (p && p.color) || "#8A8A91";
    const letter = String((p && p.name) || "?").trim().charAt(0).toUpperCase();
    if (p && p.logo) {
      return `<span class="avatar" style="width:${sz}px;height:${sz}px;background:${bg}"><i class="logo" style="-webkit-mask-image:url(logos/${p.logo});mask-image:url(logos/${p.logo});width:${Math.round(sz*0.5)}px;height:${Math.round(sz*0.5)}px"></i></span>`;
    }
    return `<span class="avatar" style="width:${sz}px;height:${sz}px;background:${bg};font-size:${Math.round(sz*0.4)}px">${letter}</span>`;
  }
  ...
  root.SVSTPresets = { LIST: P, FEATURED, find, search, avatarHtml, CHANNEL, CHANNEL_URL };
